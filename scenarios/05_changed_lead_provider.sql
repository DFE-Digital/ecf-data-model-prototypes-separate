...
select d.*, lp.name as lead_provider_name, te.name as teacher_name
...
inner join teachers te on te.id = t.teacher_id
inner join declarations d on t.id = d.training_id
where lp.id = 1;

-- and for a given participant
select d.*, lp.name as lead_provider_name, te.name as teacher_name
from lead_providers lp
inner join provider_partnerships pp on lp.id = pp.lead_provider_id
inner join training t on pp.id = t.provider_partnership_id
...
select d.*, lp.name, ct.teacher_name
...
-- when "current" (this is a bit lame, need to determine current-ness)
select d.*, lp.name as lead_provider_name, te.name as teacher_name
from lead_providers lp
inner join provider_partnerships pp on lp.id = pp.lead_provider_id
inner join training t on pp.id = t.provider_partnership_id
